...
namespace ExcelDataReader
{
    ...
    public class ExcelDataReader
    {
        ...
        public ExcelDataReader(string dirFile) { 
            FileStream fs = new FileStream(dirFile,FileMode.Open, FileAccess.Read);
        ...
        public ExcelDataReader(byte[] oribuffer) {

            m_file = new MemoryStream(oribuffer); // new BufferedStream(file);
            m_hdr = XlsHeader.ReadHeader(m_file);
            XlsRootDirectory dir = new XlsRootDirectory(m_hdr);
            XlsDirectoryEntry workbookEntry = dir.FindEntry("Workbook");
            if (workbookEntry == null)
        ...
        public DataTable TruColumnsTransform(DataTable vdT)
        {
            for (int i = 0; i < vdT.Columns.Count; i++)
            {
                if (!String.IsNullOrEmpty(vdT.Rows[0][i].ToString().ToLower()))
                {
                    vdT.Columns[i].ColumnName = vdT.Rows[0][i].ToString().ToLower();
                }
